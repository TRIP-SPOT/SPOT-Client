...
export default function PopularSpot() {
  return (
    <View className="mt-4 flex flex-col items-center justify-center px-2 pb-4">
      ...
      <View className="mt-4">
        <Font type="body2" color="black">
          구룡포 일본인 가옥거리는 '동백꽃 필 무렵'의 주요 촬영지 중 하나로,
          드라 마의 포스터에도 등장한 바로 그 장소입니다. 이곳에는 주인공
          동백이가 운영하던 까멜리아도 자리하고 있어, 드라마 속 장면들이
          현실에서 그대로 펼쳐지는 듯한 분위기를 자아냅니다. 좁은 골목길과
          고즈넉한 건물들이 어우러져 동백이의 일상과 따뜻한 사랑 이야기가
          그려졌던 공간의 감성을 고스란히 느낄 수 있으며, 동백이의 삶 이 우리
          눈앞에 펼쳐지는 듯한 특별한 경험을 할 수 있습니다.
        </Font>
      </View>
      {CONTENTS.map((item) => (
        <View key={item.title}>
          <ContentTitle mainTitle={item.title} subTitle={item.subtitle} />
          <ContentImage type="wide" asset={item.image} />
          <View className="mt-4">
            <Font type="body2" color="black">
              {item.content}
            </Font>
